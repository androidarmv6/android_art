...
LIR* ArmMir2Lir::LoadConstantWide(RegStorage r_dest, int64_t value) {
  ...
  if (r_dest.IsFloat()) {
    ...
    NewLIR2(kThumb2Vmovs_IMM8, r_dest.GetReg()*2, r_temp.GetReg());
    LoadConstantNoClobber(r_temp, val_hi);
    res = NewLIR2(kThumb2Vmovs_IMM8, r_dest.GetReg()*2+1, r_temp.GetReg());
    FreeTemp(r_temp);
#endif
  } else {
...
#else
    LIR* lir;
    if ((r_dest.GetLowReg() & 1) == 0 && r_dest.GetLowReg() < 12) { // Assumption: rt != 15
      if (r_dest.GetLowReg()+1 != r_dest.GetHighReg()) {
        ...
        FreeTemp(RegStorage(RegStorage::k32BitSolo, r_dest.GetLowReg()+1));
      } else {
      ...
      NewLIR2(kThumb2MovRR, r_dest.GetLowReg(), 0);
      NewLIR2(kThumb2MovRR, r_dest.GetHighReg(), 1);
...
LIR* ArmMir2Lir::StoreBaseDisp(RegStorage r_base, int displacement, RegStorage r_src,
  ...
  if (UNLIKELY(is_volatile == kVolatile &&
    ...
    LIR* fail_target = NewLIR0(kPseudoTargetLabel);
    NewLIR3(kThumb2Ldrexd, 0, 1, r_ptr.GetReg());
    NewLIR4(kThumb2Strexd, 0, 1, 2, r_ptr.GetReg());
    OpCmpImmBranch(kCondNe, RegStorage(RegStorage::k32BitSolo, 0), 0, fail_target);
    NewLIR2(kThumb2MovRR, r_src.GetLowReg(), 1);
    NewLIR2(kThumb2MovRR, r_src.GetHighReg(), 2);
