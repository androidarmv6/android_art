...
void ArmMir2Lir::GenLargeSparseSwitch(MIR* mir, uint32_t table_offset, RegLocation rl_src) {
  ...
  if (mod_imm > 0) {
    ...
    NewLIR3(kThumbAddRRR, r_base.GetReg(), rs_rARM_PC.GetReg(), tmp2.GetReg());
    FreeTemp(tmp2);
  }
#endif
  // Set up r_idx
...
void ArmMir2Lir::GenLargePackedSwitch(MIR* mir, uint32_t table_offset, RegLocation rl_src) {
  ...
  if (mod_imm > 0) {
    ...
    RegStorage tmp2 = AllocTemp();
    LoadConstantNoClobber(tmp2, WrapPointer(tab_rec));
    NewLIR3(kThumbAddRRR, table_base.GetReg(), rs_rARM_PC.GetReg(), tmp2.GetReg());
    FreeTemp(tmp2);
  }
#endif
...
void ArmMir2Lir::GenFillArrayData(uint32_t table_offset, RegLocation rl_src) {
  ...
  if (mod_imm > 0) {
    ...
    RegStorage tmp2 = AllocTemp();
    LoadConstantNoClobber(tmp2, WrapPointer(tab_rec));
    NewLIR3(kThumbAddRRR, rs_r1.GetReg(), rs_rARM_PC.GetReg(), tmp2.GetReg());
    FreeTemp(tmp2);
  }
#endif
  ClobberCallerSave();
...
void ArmMir2Lir::GenMonitorEnter(int opt_flags, RegLocation rl_src) {
  ...
  if (kArchVariantHasGoodBranchPredictor) {
    ...
    NewLIR4(kThumb2Strex/*eq*/, rs_r1.GetReg(), rs_r2.GetReg(), rs_r0.GetReg(),
        mirror::Object::MonitorOffset().Int32Value() >> 2);
#else
    LIR* it = OpIT(kCondEq, "TT");
    NewLIR3(kThumb2AddRRI12/*eq*/, rs_r0.GetReg(), rs_r0.GetReg(), mirror::Object::MonitorOffset().Int32Value());
    NewLIR3(kThumb2Strex/*eq*/, rs_r1.GetReg(), rs_r2.GetReg(), rs_r0.GetReg());
    NewLIR3(kThumb2SubRRI12/*eq*/, rs_r0.GetReg(), rs_r0.GetReg(), mirror::Object::MonitorOffset().Int32Value());
#endif
    OpEndIT(it);
    OpRegImm(kOpCmp, rs_r1, 0);
    it = OpIT(kCondNe, "T");
