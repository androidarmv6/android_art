...
#else
    bool isTemp = GetRegInfo(RegStorage(RegStorage::k32BitSolo, r_tmp.GetReg()+1))->IsTemp();
    if ((r_tmp.GetReg() & 1) != 0 || r_tmp.GetRegNum() > 13 || isTemp) {
      NewLIR1(kThumb2Push, 3);
      NewLIR3(kThumb2Ldrexd, 0|RegStorage::k32BitSolo, 1|RegStorage::k32BitSolo, r_ptr.GetReg());
      NewLIR2(kThumb2MovRR, r_tmp.GetReg(), 0|RegStorage::k32BitSolo);
      NewLIR2(kThumb2MovRR, r_tmp_high.GetReg(), 1|RegStorage::k32BitSolo);
      NewLIR1(kThumb2Pop, 3);
    } else {
      if (r_tmp.GetRegNum()+1 != r_tmp_high.GetRegNum()) {
        ...
        UnmarkTemp(RegStorage(RegStorage::k32BitSolo, r_tmp.GetReg()+1));
      } else {
        NewLIR3(kThumb2Ldrexd, r_tmp.GetReg(), r_tmp_high.GetReg(), r_ptr.GetReg());
      }
    }
...
#else
      NewLIR3(kThumbOrr, r_tmp.GetReg(), r_tmp.GetReg(), r_tmp_high.GetReg());
#endif
    } else {
...
#else
    isTemp = GetRegInfo(RegStorage(RegStorage::k32BitSolo, rl_new_value.reg.GetLowReg()+1));
    if (rl_new_value.reg.GetLowReg()+1 == rl_new_value.reg.GetHighReg()) {
      it = OpIT(kCondEq, "T");
      NewLIR4(kThumb2Strexd /* eq */, r_tmp.GetReg(), rl_new_value.reg.GetLowReg(), rl_new_value.reg.GetHighReg(), r_ptr.GetReg());
    } else if (isTemp) {
      it = OpIT(kCondEq, "TTT");
      NewLIR1(kThumb2Push, 1<<(rl_new_value.reg.GetLowReg()+1));
      NewLIR4(kThumb2Strexd /* eq */, r_tmp.GetReg(), rl_new_value.reg.GetLowReg(), rl_new_value.reg.GetLowReg()+1, r_ptr.GetReg());
      NewLIR2(kThumb2MovRR /* eq */, rl_new_value.reg.GetHighReg(), rl_new_value.reg.GetLowReg()+1);
      NewLIR1(kThumb2Pop, 1<<(rl_new_value.reg.GetLowReg()+1));
      OpEndIT(it);
      it = OpIT(kCondEq, "");
    } else {
      it = OpIT(kCondEq, "TT");
      ...
      NewLIR4(kThumb2Strexd /* eq */, r_tmp.GetReg(), rl_new_value.reg.GetLowReg(), rl_new_value.reg.GetLowReg()+1, r_ptr.GetReg());
      NewLIR2(kThumb2MovRR /* eq */, rl_new_value.reg.GetHighReg(), rl_new_value.reg.GetLowReg()+1);
      UnmarkTemp(RegStorage(RegStorage::k32BitSolo, rl_new_value.reg.GetLowReg()+1));
    }
#endif
  } else {
#ifndef ARM_MODE_WORKAROUND
...
#else
  NewLIR3(kThumbSubRI8, rs_rARM_SUSPEND.GetReg(), rs_rARM_SUSPEND.GetReg(), 1);
#endif
